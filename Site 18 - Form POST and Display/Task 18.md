# Task 18: Form POST and Data Processing

## Technical Explanation

This site focuses on building a two-page form flow using POST parameters and PHP string processing. Students will:

1. Create an HTML form on page 1 with `method="POST"`
2. Collect user input from form fields
3. Submit to page 2 in the request body
4. Receive data in PHP using `$_POST` with null coalescing
...
2. Users can submit the form without exposing the entered values in the URL.
...
5. Users can understand the complete flow: form → POST → PHP variables → display with validation.
...
| FR  | Requirement | Status |
|-----|-------------|--------|
| FR8 | Receive POST data via POST array with null coalescing on page 2 | Required |
...
1. HTML form `method="POST"` passes data in the request body
2. Form field `name` attributes become POST parameter keys
...
5. Null coalescing operator (`??`) provides defaults if POST parameters are missing
...
- `form_output.php` - page 2 with POST processing, validation, and output
...
1. Display an HTML form with the submission method set to POST and the action pointing to form_output.php.
...
5. Read the first_name field from the POST data, using an empty string as the default if it is absent.
6. Read the last_name field from the POST data in the same way.
...
- **`yourtask_form.php`** — create this file yourself. It is the form page (page 1) that collects user input and submits it with `method="POST"` to `yourtask.php`.
- **`yourtask.php`** — already provided. It is the processing page (page 2) that reads `$_POST` values, validates them, and displays the results.
...
- Use `$_POST['field_name'] ?? ''` to safely receive form values with defaults
...
| FORM-4 | Fill form with "Alice", "Smith", "Gaming", "Blue" and submit | Page 2 loads and the URL does not expose the form fields |
...
| FORM-7 | Submit form with server-side bypass (curl -X POST) with invalid length | Page 2 shows validation error message |
...
```bash
/usr/bin/php -S 0.0.0.0:8000 -t "Site 18 - Form POST and Display"
```
...
| ID | Requirement |
|---|---|
| FR1 | a website with multiple pages, one of which has form elements (action, method, and name) to collect user input using POST |
| FR2 | N/A |
